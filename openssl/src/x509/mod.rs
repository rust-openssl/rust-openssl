...
impl X509CrlBuilder {
    ...
    pub fn set_last_update(&mut self, t: &Asn1TimeRef) -> Result<(), ErrorStack> {
        unsafe { cvt(X509_CRL_set1_lastUpdate(self.0.as_ptr(), t.as_ptr())).map(|_| ()) }
    }
    ...
    pub fn set_next_update(&mut self, t: &Asn1TimeRef) -> Result<(), ErrorStack> {
        unsafe { cvt(X509_CRL_set1_nextUpdate(self.0.as_ptr(), t.as_ptr())).map(|_| ()) }
    }
...
cfg_if! {
    if #[cfg(any(ossl110, libressl, boringssl, awslc))] {
        use ffi::{
            ...
            X509_CRL_get_REVOKED, X509_CRL_set1_lastUpdate, X509_CRL_set1_nextUpdate,

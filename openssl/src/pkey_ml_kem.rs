...
impl Variant {
    ...
    pub fn public_key_bytes(&self) -> usize {
        match self {
            ...
            #[cfg(boringssl)]
            Variant::MlKem768 => MLKEM768_PUBLIC_KEY_BYTES,
            #[cfg(boringssl)]
            Variant::MlKem1024 => MLKEM1024_PUBLIC_KEY_BYTES,
            #[cfg(ossl350)]
    ...
    pub fn ciphertext_bytes(&self) -> usize {
        match self {
            ...
            Variant::MlKem512 => panic!("ML-KEM-512 not supported by BoringSSL"),
            #[cfg(boringssl)]
            Variant::MlKem768 => MLKEM768_CIPHERTEXT_BYTES,
            #[cfg(boringssl)]
            Variant::MlKem1024 => MLKEM1024_CIPHERTEXT_BYTES,
            #[cfg(ossl350)]
...
pub struct PKeyMlKemParams<T> {
    ...
    seed: Option<[u8; MLKEM_SEED_BYTES]>,
    _m: PhantomData<T>,
}
...
impl PKeyMlKemParams<Private> {
        ...
        match variant {
            ...
            Variant::MlKem768 => {
                let mut priv_key = ffi::MLKEM768_private_key {
                    opaque: ffi::MLKEM768_private_key__bindgen_ty_1 { alignment: 0 },
                };
                unsafe {
            ...
            Variant::MlKem1024 => {
                let mut priv_key = ffi::MLKEM1024_private_key {
                    opaque: ffi::MLKEM1024_private_key__bindgen_ty_1 { alignment: 0 },
                };
                unsafe {
    ...
    pub fn from_seed(variant: Variant, seed: &[u8]) -> Result<Self, ErrorStack> {
            ...
            Variant::MlKem768 => {
                ...
                unsafe {
                    ...
                    let mut pub_key = ffi::MLKEM768_public_key {
                        opaque: ffi::MLKEM768_public_key__bindgen_ty_1 { alignment: 0 },
                    };
                    ...
                    cvt(ffi::MLKEM768_marshal_public_key(&mut cbb, &pub_key))?;
            ...
            Variant::MlKem1024 => {
                ...
                unsafe {
                    ...
                    let mut pub_key = ffi::MLKEM1024_public_key {
                        opaque: ffi::MLKEM1024_public_key__bindgen_ty_1 { alignment: 0 },
                    };
                    ...
                    let mut cbb: ffi::cbb_st = std::mem::zeroed();
                    cvt(ffi::CBB_init(&mut cbb, variant.public_key_bytes()))?;
                    cvt(ffi::MLKEM1024_marshal_public_key(&mut cbb, &pub_key))?;
        ...
        let mut seed_arr = [0u8; MLKEM_SEED_BYTES];
    ...
    pub fn decapsulate(&self, ciphertext: &[u8]) -> Result<Vec<u8>, ErrorStack> {
        ...
        let mut shared_secret = vec![0u8; MLKEM_SHARED_SECRET_BYTES];
...
mod tests_boringssl {
    ...
    fn test_generate(variant: Variant) {
        ...
        assert_eq!(secret1.len(), MLKEM_SHARED_SECRET_BYTES);
    ...
    fn test_from_seed(variant: Variant) {
        let seed = [0x42u8; MLKEM_SEED_BYTES];

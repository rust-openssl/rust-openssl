...
impl CipherCtxRef {
    ...
    fn is_aead(&self) -> bool {
        unsafe {
            ...
            ffi::EVP_CIPHER_flags(cipher) & ffi::EVP_CIPH_FLAG_AEAD_CIPHER != 0
        }
    }

    #[cfg(any(boringssl, awslc))]
    fn is_aead(&self) -> bool {
        // EVP_CIPHER_flags isn't a reliable AEAD signal on these backends, so
        // assume AEAD to keep existing behavior rather than reject valid usage.
        true
    }
    ...
    /// That is only meaningful for AEAD ciphers, so passing no output buffer with a non-AEAD cipher returns
    /// an error.
    ///
    ...
    pub fn cipher_update(
        ...
        if output.is_none() && !self.is_aead() {
...
mod test {
    ...
    #[test]
    fn no_output_buffer_rejected_for_non_aead() {
        // Passing no output buffer feeds the input as AAD, which only AEAD
        // ciphers understand. With a non-AEAD cipher OpenSSL used to encrypt
        // into the null output pointer and segfault, so this must error.
        let mut ctx = CipherCtx::new().unwrap();
        ctx.encrypt_init(Some(Cipher::aes_128_cbc()), Some(&[0; 16]), Some(&[0; 16]))
            .unwrap();
        assert!(ctx.cipher_update(&[0; 16], None).is_err());
    }

    #[test]
    fn no_output_buffer_allowed_for_aead() {
        // The AEAD case must keep working: feeding AAD with no output buffer is
        // the documented way to authenticate additional data.
        let mut ctx = CipherCtx::new().unwrap();
        ctx.encrypt_init(Some(Cipher::aes_128_gcm()), Some(&[0; 16]), Some(&[0; 12]))
            .unwrap();
        assert!(ctx.cipher_update(&[0; 16], None).is_ok());
    }
}
